...
- Đầu vào và phiên bản: B1 khảo sát công khai `APPROVED` ngày 2026-08-13; B1 chẩn đoán `APPROVED` ngày 2026-08-13; A1/A2/A4 baseline `APPROVED` ngày 2026-08-13; PRJ-001–PRJ-002 và BIZ-001–BIZ-091 trong sổ quyết định
...
| **Quản trị viên** (*admin*) | Người kiểm duyệt, nhập tỷ lệ phí nền tảng theo thỏa thuận, xác nhận hủy khi cần và đánh dấu chi trả thủ công. | **Nhà tổ chức:** không có quyền quản trị toàn nền tảng. | `CANDIDATE` |
...
| **Trạng thái sự kiện** | Tình trạng vòng đời quản trị như đang soạn, chờ duyệt, được duyệt, bị từ chối, đã công bố hoặc đã hủy; chuyển trạng thái chi tiết thuộc B3. | **Trạng thái mở bán:** kết quả xét điều kiện giao dịch tại một thời điểm. | `CANDIDATE` |
...
| **Hạn mức bán** | Số lượng tối đa được bán của loại vé hoặc sector đứng. | **Tồn kho còn lại:** phần chưa bán hoặc giữ hợp lệ. | `CANDIDATE` |
| **Tồn kho vé còn lại** | Lượng quyền bán còn có thể giữ sau khi xét phần đã bán và đang giữ hợp lệ. | **Danh sách vé:** tập quyền đã có định danh. | `CANDIDATE` |
| **Khả dụng** | Kết quả xét ghế hoặc số lượng có thể chọn dựa trên trạng thái bán, tồn kho, giữ chỗ và giới hạn mua. | **Tồn kho:** chỉ là một đầu vào. | `CANDIDATE` |
...
| **Hết vé** | Không còn lượng khả dụng cho ý định mua mới tại thời điểm xét. | **Ngừng bán:** có thể do trạng thái/thời gian dù còn tồn kho. | `CANDIDATE` |
...
| **Callback/IPN** | Thông báo kết quả từ cổng thanh toán; thông báo lặp không tạo tác dụng phụ lặp. | **Xác nhận thanh toán:** sự thật nghiệp vụ sau xác minh. | `CANDIDATE` |
...
| **Thanh toán đến muộn** | Xác nhận thu tiền đến sau khi đơn hết hạn; khoản thu được hoàn toàn bộ mà không giải phóng tồn kho hoặc giới hạn lần nữa. | **Thanh toán thất bại:** không có tiền đã thu. | `USER_CONFIRMED` |
...
| **Mã vé/QR** | Dữ liệu hoặc khóa tham chiếu để xác định vé khi vào cửa. | **Vé:** quyền nghiệp vụ. | `CANDIDATE` |
...
| **QR đã vô hiệu** | QR của vé đã hoàn, hủy hoặc vô hiệu không còn được xem/tải; vé vẫn hiện trong lịch sử buyer với trạng thái tương ứng. | **Xóa lịch sử:** sản phẩm vẫn giữ dấu vết vé. | `USER_CONFIRMED` |
...
| Thuật ngữ | Định nghĩa thống nhất | Phân biệt dứt khoát với | Trạng thái |
|---|---|---|---|
| **Yêu cầu hoàn tiền** | Hồ sơ trả lại toàn bộ khoản thực thu đủ điều kiện của một đơn; không hỗ trợ hoàn một phần. | **Hoàn tiền:** kết quả dịch chuyển tiền. | `USER_CONFIRMED` |
| **Hoàn tiền** | Trả toàn bộ khoản cần hoàn và ghi trạng thái tương ứng của đơn/vé. | **Hủy đơn:** đơn chưa thu tiền không tạo hoàn tiền. | `USER_CONFIRMED` |
...
| **Phí nền tảng** | Tỷ lệ phần trăm admin nhập khi duyệt theo thỏa thuận ngoài hệ thống, tính trên doanh thu thực thu. | **Số tiền chi trả:** phần ròng cho organizer. | `USER_CONFIRMED` |
...
| Thuật ngữ | Định nghĩa thống nhất | Phân biệt dứt khoát với | Trạng thái |
|---|---|---|---|
| **Sự cố** | Tình huống hệ thống không cung cấp hoặc có nguy cơ không cung cấp hành vi mong đợi, cần con người xác minh. | **Lỗi log:** một bản ghi, không tự là một sự cố. | `CANDIDATE` |
| **Lỗi âm thầm** | Hành vi sai hoặc luồng đình trệ nhưng không tạo ngoại lệ rõ ràng tại nơi quan sát đầu tiên. | **Lỗi ngoại lệ:** có exception trực tiếp. | `CANDIDATE` |
| **Lỗi ngoại lệ ứng dụng** | Sự cố có exception trong quá trình thực thi ứng dụng. | **Nguyên nhân gốc:** exception chưa chắc là nguyên nhân cuối. | `CANDIDATE` |
| **Lỗi cơ sở dữ liệu** | Sự cố liên quan truy cập, ràng buộc, kết nối hoặc trạng thái dữ liệu. | **Từ chối nghiệp vụ:** hành vi đúng quy tắc. | `CANDIDATE` |
| **Lỗi hạ tầng/tích hợp** | Sự cố do tài nguyên, mạng, tiến trình hoặc hệ thống phụ thuộc. | **Lỗi ứng dụng:** phát sinh trong logic. | `CANDIDATE` |
| **Dấu vết vận hành** | Log, trace, exception và metadata dùng để dựng lại diễn biến. | **Nguyên nhân gốc:** dấu vết là bằng chứng đầu vào. | `CANDIDATE` |
| **Log có cấu trúc** | Bản ghi có trường và định dạng nhất quán để máy xử lý. | **Thông điệp log:** chỉ là phần văn bản. | `CANDIDATE` |
| **Mã tương quan** | Định danh liên kết bản ghi thuộc cùng yêu cầu hoặc luồng nghiệp vụ. | **Mã nghiệp vụ:** định danh đơn, vé, thanh toán. | `CANDIDATE` |
| **Mẫu log Drain** | Cấu trúc ổn định rút ra sau khi tách phần biến đổi của các thông điệp tương tự. | **Nguyên nhân:** mẫu log không tự kết luận nguyên nhân. | `CANDIDATE` |
| **Context chẩn đoán** | Dấu vết đã chọn, liên kết, rút gọn và khử nhạy cảm cho một sự cố. | **Toàn bộ log:** context chỉ giữ phần liên quan. | `CANDIDATE` |
| **Nguyên nhân khả dĩ** | Giả thuyết phù hợp bằng chứng nhưng chưa được con người xác minh. | **Nguyên nhân gốc đã xác nhận:** kết luận cuối. | `CANDIDATE` |
| **Bước kiểm tra tiếp theo** | Hành động đọc/xác minh để củng cố hoặc loại trừ giả thuyết. | **Tự sửa:** trợ lý không chạy lệnh hoặc ghi nghiệp vụ. | `CANDIDATE` |
...
|---|---|---|---|
| **Sự kiện miền** | Phát biểu ở thì quá khứ về một việc nghiệp vụ đã xảy ra, dùng lập dòng thời gian ở B4. | **Sự kiện bán vé:** đối tượng được tổ chức; **lệnh:** yêu cầu một việc xảy ra. | `CANDIDATE` |
| **Trạng thái nghiệp vụ** | Tình trạng hiện tại có ý nghĩa với quy tắc và hành vi miền. | **Trạng thái kỹ thuật:** tình trạng tiến trình hoặc kết nối. | `CANDIDATE` |
| **Bất biến nghiệp vụ** | Điều kiện phải luôn được bảo vệ kể cả khi đồng thời, thử lại hoặc lỗi từng phần. | **Mục tiêu hiệu năng:** ngưỡng mong muốn. | `CANDIDATE` |
