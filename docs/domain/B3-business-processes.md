...
- Đầu vào và phiên bản: `docs/glossary.md` — `B2-v0.4`, `APPROVED` ngày 2026-08-18; `docs/project/decision-register.md` — PRJ-001–PRJ-002 và BIZ-001–BIZ-091 ngày 2026-08-17
- Phân lớp: `FORMATION`
...
- Nhà tổ chức: tạo, sửa, gửi duyệt, công bố và yêu cầu hủy sự kiện của mình.
- Quản trị viên: duyệt/từ chối, nhập tỷ lệ phí nền tảng và xác nhận hoặc chủ động hủy trong phạm vi cho phép.
...
- Giá mọi loại vé và giới hạn mua bắt buộc lớn hơn 0.

### 2.3 Luồng chính

1. Nhà tổ chức tạo sự kiện ở trạng thái `DRAFT`, nhập nội dung, thời gian, địa điểm, cửa sổ bán, giới hạn mua và `salesMode`.
2. Nhà tổ chức cấu hình loại vé và nguồn cung phù hợp với `salesMode`.
3. Khi gửi duyệt, cấu hình thương mại và `salesMode` bị khóa.
4. Quản trị viên kiểm duyệt nội dung. Nếu chấp thuận, admin nhập tỷ lệ phần trăm phí nền tảng theo thỏa thuận đã hoàn tất ngoài hệ thống và chuyển sự kiện sang trạng thái đã duyệt.
5. Nhà tổ chức chủ động công bố sự kiện đã duyệt. Công bố chỉ làm sự kiện hiển thị, không thay đổi cửa sổ bán; lặp lại cùng yêu cầu công bố không tạo thêm tác dụng phụ.
...
- Nếu admin từ chối hoặc trả lại, sự kiện về `DRAFT`; organizer được sửa và gửi duyệt lại.
- Khi đổi `salesMode` ở `DRAFT`, organizer phải xác nhận xóa cấu hình không tương thích; hệ thống không tự chuyển đổi cấu hình.
- Organizer được hủy trực tiếp khi chưa có đơn đã thu tiền. Khi đã có đơn thu tiền, organizer chỉ gửi yêu cầu hủy và admin quyết định xác nhận. Admin có thể chủ động hủy khi cần.
...
Sự kiện bán vé, trạng thái sự kiện, thời gian/địa điểm, cửa sổ bán, `salesMode`, loại vé, sector/ghế, giới hạn mua, tỷ lệ phí nền tảng, yêu cầu hủy.

### 2.7 Quyết định nguồn

`BIZ-004`–`BIZ-010`, `BIZ-015`–`BIZ-021`, `BIZ-025`, `BIZ-034`, `BIZ-035`, `BIZ-055`–`BIZ-058`, `BIZ-083`, `BIZ-088`, `BIZ-089`.
...
4. Người mua có thể áp dụng tối đa một mã khuyến mãi thuộc đúng sự kiện của đơn. Chuỗi mã duy nhất trong phạm vi event; hệ thống kiểm tra loại giảm, thời gian hiệu lực, tổng lượt, một lần/tài khoản/mã, không cộng dồn và số tiền cuối lớn hơn 0. Lượt dùng được giữ cùng thời hạn đơn.
...
- Callback hợp lệ đến sau khi đơn hết hạn hoặc bị hủy tạo trường hợp thanh toán đến muộn: hoàn toàn bộ khoản thu; không giải phóng giữ chỗ, tồn kho, giới hạn hay lượt khuyến mãi lần thứ hai.
...
- Gửi email/QR thất bại không phải lỗi phát hành, không kích hoạt hoàn tiền; vé đã phát hành vẫn hợp lệ và buyer có thể tải/gửi lại.
- Vé đã hoàn, hủy hoặc vô hiệu vẫn hiện trong lịch sử buyer với trạng thái tương ứng nhưng không cho xem hoặc tải QR thô.
...
- Ngoại lệ sau thu tiền: nghĩa vụ hoàn được chuyển sang Quy trình 3 mà không tạo tác dụng phụ trùng.
...
`BIZ-001`–`BIZ-003`, `BIZ-011`–`BIZ-020`, `BIZ-023`–`BIZ-032`, `BIZ-039`–`BIZ-048`, `BIZ-060`–`BIZ-065`, `BIZ-069`–`BIZ-081`, `BIZ-083`, `BIZ-086`, `BIZ-090`, `BIZ-091`.
...
- Quản trị viên: xác nhận/chủ động hủy, đối chiếu báo cáo cổng thanh toán và đánh dấu đã chi trả.
...
- Hủy event và hoàn hàng loạt do hủy chỉ thuộc phạm vi trước `eventStartAt`; ba nhóm hoàn kỹ thuật của một đơn vẫn được xử lý khi phát sinh sau mốc này.

### 4.3 Luồng hoàn một đơn

1. Hệ thống xác định khoản thu cụ thể và nguyên nhân hoàn.
2. Hệ thống bảo đảm cùng một nghĩa vụ hoàn chỉ được tạo/xử lý một lần dù yêu cầu hoặc phản hồi bị lặp.
...
5. Khi có kết quả hợp lệ, hệ thống ghi trạng thái hoàn của giao dịch, đơn và vé liên quan; cập nhật dữ liệu đối soát.
...
1. Organizer gửi yêu cầu để admin xác nhận nếu đã có đơn thu tiền, hoặc admin chủ động hủy. Organizer được hủy trực tiếp khi chưa có đơn thu tiền.
2. Hệ thống chỉ chấp nhận luồng hủy tự động khi thời điểm hiện tại trước `eventStartAt`.
3. Khi hủy được xác nhận, sự kiện đóng bán; toàn bộ vé của sự kiện bị vô hiệu hóa và nguồn cung không trở lại khả dụng.
4. Mọi đơn chưa thanh toán và giữ chỗ đang hoạt động bị hủy; hệ thống không nhận lần thanh toán mới, trả phần giới hạn mua và lượt khuyến mãi đúng một lần nhưng không mở lại nguồn cung.
5. Hệ thống lấy từng đơn đã thu tiền đủ điều kiện và gọi lại quy trình hoàn một đơn. Mỗi đơn có trạng thái và khả năng thử lại độc lập; lỗi ở một đơn không làm mất dấu các đơn còn lại.
6. Callback thu tiền đến sau khi event/đơn đã hủy được hoàn toàn bộ mà không giải phóng tài nguyên lần thứ hai.
7. Kết quả tổng hợp cho biết số đơn chờ xử lý, thành công và thất bại để admin theo dõi.
...
2. `Phí nền tảng = doanh thu thực thu × tỷ lệ phí`; `số tiền chi trả = doanh thu thực thu - phí nền tảng`.
...
5. Việc chuyển tiền cho organizer diễn ra ngoài hệ thống. Sau đó admin đánh dấu `PAID` đúng một lần và lưu người thao tác, thời điểm cùng ghi chú/bằng chứng đối chiếu.
...
- Mỗi khoản thu đủ điều kiện có tối đa một kết quả hoàn thành công.
...
`BIZ-008`–`BIZ-014`, `BIZ-033`–`BIZ-038`, `BIZ-050`–`BIZ-055`, `BIZ-057`, `BIZ-059`, `BIZ-062`–`BIZ-066`, `BIZ-070`, `BIZ-083`–`BIZ-086`, `BIZ-090`, `BIZ-091`.
...
- Vé thuộc đúng sự kiện, đã phát hành, chưa hoàn/hủy/vô hiệu và chưa sử dụng.
...
- Từ chối nếu QR không xác định được vé, vé thuộc sự kiện khác, đã hoàn, đã bị vô hiệu do hủy hoặc chưa ở trạng thái hợp lệ.
...
- Mọi yêu cầu thành công hoặc bị từ chối có dấu vết đủ để kiểm tra sự kiện, vé, organizer, thiết bị, thời điểm và kết quả.
...
`BIZ-056`, `BIZ-067`, `BIZ-068`, `BIZ-073`, `BIZ-074`, `BIZ-077`, `BIZ-082`, `BIZ-087`, `BIZ-089` và phạm vi mobile/check-in trực tuyến đã duyệt.
...
| Thanh toán đến muộn, thu trùng hoặc phát hành lỗi | Quy trình 3 | Tạo nghĩa vụ hoàn toàn bộ phù hợp nguyên nhân, không phụ thuộc event đã bắt đầu hay chưa |
| Event bị hủy | Quy trình 2, 3 và 4 | Hủy đơn đang giữ, hoàn đơn đã thu, vô hiệu vé và từ chối check-in |
