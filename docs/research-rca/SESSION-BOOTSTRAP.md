...
2. Xác nhận yêu cầu mới cho phép bước nào. Hiện tại **chờ lệnh riêng bắt đầu D**; lập roadmap không cấp quyền execution.
...
**Không suy diễn:** root label ≠ anomaly-node label; operation evidence ≠ operation GT; span relation ≠ causal edge; time alignment ≠ request join; graph gain ≠ novelty; explanation ≠ sửa rank/GT. Không dùng injected-service set/path/root/fault/oracle metadata làm input. Các công thức, metric, split, baseline/threshold chưa được chọn ở C.
