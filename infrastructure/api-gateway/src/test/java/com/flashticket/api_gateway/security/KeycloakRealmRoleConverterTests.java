...
class KeycloakRealmRoleConverterTests {
  ...
  void mapsOnlyCurrentBusinessRolesFromRealmAccess() {
    Jwt jwt =
        jwtWithRealmRoles("offline_access", "BUYER", "uma_authorization", "ORGANIZER", "ADMIN");

    assertEquals(
        List.of("ROLE_BUYER", "ROLE_ORGANIZER", "ROLE_ADMIN"),
        authorityNames(converter.convert(jwt)));
  }

  @Test
  void ignoresMissingOrMalformedRealmAccessRoles() {
    Jwt missingRoles =
        Jwt.withTokenValue("token").header("alg", "none").claim("realm_access", Map.of()).build();
    Jwt malformedRoles =
        Jwt.withTokenValue("token")
            .header("alg", "none")
            .claim("realm_access", Map.of("roles", "BUYER"))
            .build();
